...
export const candidataStore = async (req, res) => {
  ...
  const { nome, idade, clube, admin_id } = req.body;

  // se não informou estes atributos
  if (!nome || !idade || !clube || !foto || admin_id) {
    res
      .status(400)
      .json({ id: 0, msg: "Erro... informe nome, idade, clube, admin_id e foto da candidata" });
    return;
  }

  try {
    const novo = await dbKnex("candidatas").insert({ nome, idade, clube, foto, admin_id });
...
export const candidataUpdate = async (req, res) => {
  ...
  try {
    await dbKnex("candidatas").where({ id }).update({ nome, idade, clube });
